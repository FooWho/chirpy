...
const setUserToRed = `-- name: SetUserToRed :one
UPDATE users SET is_chirpy_red = TRUE WHERE id = $1
RETURNING id, created_at, updated_at, email, hashed_password, is_chirpy_red
`

func (q *Queries) SetUserToRed(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, setUserToRed, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
